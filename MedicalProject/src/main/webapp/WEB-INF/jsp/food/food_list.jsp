...
<form class="form-inline definewidth m20" action="/tfood/tofoodlsit" method="post" id="submitform"> 
    ...
    <button type="submit" class="btn btn-primary">查询</button>&nbsp;&nbsp; <button type="button" class="btn btn-success" id="addnew">新增饮食记录</button>
</form>
<table class="table table-bordered table-hover definewidth m10" >
    ...
    <tr>
        ...
        <th style="text-align:center;">饮食分类</th>
    ...
    <c:if test="${fn:length(result) > 0}" var="countnumber">
    	<c:forEach items="${result }" var="h" varStatus="status">
	    	<tr>
	            ...
	            <td style="text-align:center;">
	            <a href="##" onclick="delrecode('${h.id}');">删除</a>  ｜  <a href="##" onclick="detailrecode();">详情</a>
	            </td>
	        </tr>
    	</c:forEach>
    </c:if>
...
<script>
    $(function () {
		...
		$('#addnew').click(function(){
				window.location.href="/tfood/toAddFoodItem?f="+Math.random();
		 });
    });
  	//删除记录
	function delrecode(_id)
	{
		if(confirm("确定要删除吗？"))
		{
			 $.get("/tfood/delFoodItem", {"id":_id}, function(data){
	    		  if(data === '0'){
	    			  alert("删除成功！");
